...
/**
 * Class to encapsulate the MP4 media stream information box (minf).
 * 
 * @author Anthony Campbell - anthonycampbell.co.uk
 */
public class MinfBox extends AbstractBox implements Box {
	
	// Log
	private static final Logger log = LoggerFactory.getLogger(MinfBox.class.getName());
	 ...
	 */
	public MinfBox(final MP4Reader reader, final long remainingOffset, final String boxName,
		...
		while (bytesRemaining() > 0) {
			...
			// Validate
			if (nextBox != null) {
				if (nextBox instanceof StblBox && BoxType.SAMPLE_TABLE == nextBox.getBoxType()) {
					final StblBox stblBox = (StblBox) nextBox;
